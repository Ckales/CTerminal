...
use crate::{local, schemes, serial, ssh, telnet};
...
pub fn open(profile: &Profile, config: &Config, size: WinSize, sink: EventSink) -> Arc<Session> {
    ...
    match &profile.kind {
        ProfileKind::Local(options) => match local::spawn(options, size, tx.clone()) {
        ...
        ProfileKind::Telnet(options) => {
            ...
            std::thread::spawn(move || match telnet::connect(&options, size, tx.clone()) {
                Ok(transport) => session_for_connect.attach(Box::new(transport)),
                Err(err) => {
        ...
        ProfileKind::Serial(options) => match serial::open(options, tx.clone()) {
            Ok(transport) => session.attach(Box::new(transport)),
