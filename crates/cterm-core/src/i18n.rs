...
fn english(source: &str) -> Option<&'static str> {
    let translated = match source {
        ...
        "配置文件由更新版本的 CTerminal 创建（配置版本 {version}），请升级后再打开" => "Config file was created by a newer CTerminal (config version {version}). Please upgrade to open it",
        "备份原配置文件失败：{err}" => "Failed to back up the original config file: {err}",
        ...
        "经由代理命令连接 {host}:{port} …" => "Connecting to {host}:{port} via proxy command …",
        "无法启动代理命令：{err}" => "Failed to start the proxy command: {err}",
        "经代理命令连接失败：{err}" => "Connection through the proxy command failed: {err}",
        "代理命令含不支持的占位符：%{token}" => "Unsupported token in the proxy command: %{token}",
        ...
        "检查更新失败：{err}" => "Update check failed: {err}",
        "检查更新失败：返回内容无法识别" => "Update check failed: unrecognized response",
        "ZMODEM：对方在等待上传，请选择文件…" => "ZMODEM: the remote side is waiting for an upload, choose files…",
        "ZMODEM 已取消" => "ZMODEM cancelled",
        "ZMODEM 传输失败：{err}" => "ZMODEM transfer failed: {err}",
        "对方取消了传输" => "The remote side cancelled the transfer",
        "对方没有响应" => "The remote side stopped responding",
        "对方请求了不存在的文件数据" => "The remote side asked for data of a file that is not being sent",
        "收到了不属于任何文件的数据" => "Received data that belongs to no file",
        "文件在上传过程中被截短了" => "The file was truncated during the upload",
        "已上传" => "uploaded",
        "已保存到 {path}" => "saved to {path}",
        "{path} 超过 4GB，ZMODEM 不支持" => "{path} is larger than 4 GB, which ZMODEM does not support",
        "写入 {path} 失败：{err}" => "Failed to write {path}: {err}",
        "协议错误：{err}" => "Protocol error: {err}",
        "无法创建 {path}：{err}" => "Cannot create {path}: {err}",
        "无法创建目录 {path}：{err}" => "Cannot create directory {path}: {err}",
        "无法打开 {path}：{err}" => "Cannot open {path}: {err}",
